...
Como intento final, se reentrenó el clasificador con **data augmentation** (`calibracion/acustico_ml_robusto.py`): ruido blanco SNR 10-30dB + variación de ganancia, aplicado solo a `train` (3 versiones por llamada — original + 2 aumentadas), bajo la hipótesis de que forzar al modelo a ver variación durante el entrenamiento le impediría depender del artefacto de banda estrecha ya identificado. Antes de entrenar se confirmó que la augmentation sí perturba las features de forma no trivial (diferencia media 156.5, diferencia relativa 7.16x el valor original, 0/120 features sin cambio) — descartando que un resultado nulo se debiera a dilución en la agregación media/varianza.

El resultado sobre las 71 llamadas de `val` fue **accuracy idéntica** a la del modelo sin augmentation (0.972 LR, 0.958 RF) — pero la prueba de robustez (`calibracion/probar_robustez_ml_robusto.py`, mismas 3 degradaciones, corrida sobre las 71 llamadas completas) reveló por qué: bajo ruido blanco, el score de las 71 llamadas colapsa de nuevo a un valor prácticamente constante (`0.002`, antes `0.088`) — el modelo aprendió una frontera de decisión distinta pero igual de dependiente de un atajo. Peor aún, bajo downsample (una degradación que NO formó parte del entrenamiento con augmentation), el porcentaje de llamadas que cambian de clasificación subió de 0-2/10 a **32/71 (45%)** — la augmentation con un tipo de ruido específico no transfiere robustez a otros tipos de degradación, y puede incluso empeorarla.

**Conclusión**: se descarta también este enfoque. El patrón se repite en **cuatro intentos distintos** de acústica (heurística, modelo pre-entrenado wav2vec2, ML propio, ML con data augmentation) — confirma que el límite es estructural al dominio (información espectral insuficiente/no confiable en 8kHz telefónico de este dataset específico), no una limitación de la técnica de entrenamiento usada. Código en `calibracion/acustico_ml_robusto.py` y `calibracion/probar_robustez_ml_robusto.py`.
...
Con esto, la señal acústica fue explorada con **cuatro enfoques distintos** (heurística MFCC simple, modelo pre-entrenado wav2vec2, ML entrenado propio sobre features MFCC+delta+delta2, y ese mismo ML con data augmentation) — los cuatro fallaron la misma prueba de robustez ante degradación acústica, confirmando que el límite es estructural (información espectral insuficiente/no confiable en telefonía 8kHz de este dataset), no una limitación de un enfoque o técnica de entrenamiento particular.
...
- **Señal acústica descartada con cuatro enfoques distintos, no arreglada**: la heurística MFCC (`app/deteccion/acustico.py`), la alternativa wav2vec2 (`calibracion/acustico_wav2vec.py`), un clasificador de ML propio entrenado sobre features MFCC+delta+delta2 (`calibracion/acustico_ml_entrenado.py`, accuracy 0.972 en val) y ese mismo ML reentrenado con data augmentation (ruido + ganancia, `calibracion/acustico_ml_robusto.py`) se probaron con datos reales. Los dos ML propios parecían prometedores hasta someterlos a la misma prueba de robustez ante degradación acústica que ya se aplica a `comportamiento`: ante ruido blanco (SNR~20dB) las llamadas de prueba colapsan al mismo score exacto sin importar su valor original (con o sin augmentation — solo cambia la constante), y ante códec/sub-muestreo cambia clasificación en hasta 45% de las 71 llamadas de val con augmentation (vs. 0/10 de `comportamiento`) — evidencia de sobreajuste al pipeline de generación del dataset (*shortcut learning*), no de una señal robusta de síntesis de voz, y de que la augmentation con un tipo de ruido específico no transfiere robustez a otros tipos de degradación. El código de los cuatro enfoques sigue corriendo o disponible (con peso 0 en producción, no afecta el resultado) por si se quiere reintentar más adelante con otro enfoque o dataset.
...
│   ├── acustico_ml_robusto.py        # mismo ML + data augmentation, probado y descartado
│   ├── probar_robustez_ml_robusto.py # prueba de robustez que descartó acustico_ml_robusto.py
